...
- Zeitzone ist standardmäßig `Europe/Berlin`. `TZ` allein reicht nicht, wenn die Anwendung eine eigene Zeitzonen-Einstellung hat (z. B. `APP_TIMEZONE` bei Laravel-Apps, `GENERIC_TIMEZONE` bei n8n, `PAPERLESS_TIME_ZONE`) – diese wird zusätzlich gesetzt, sonst zeigt die Oberfläche UTC.
- **Image-Tags:** Anwendungen und Datenbanken werden bevorzugt auf die **Major-Version** gepinnt (z. B. `postgres:18`, `redis:8`, `vikunja:2`). Bietet das Projekt keinen floatenden Major-Tag an — etwa weil es nur volle Semver-Tags oder ein Datums-/CalVer-Schema (`2026.6.1`) veröffentlicht —, wird `latest` verwendet. Ungetaggte Images (implizit `latest`) sind zu vermeiden. Ausnahme: Images, die eine bestimmte, vom Upstream vorgegebene kompatible Version voraussetzen (z. B. das für Immich vorbereitete PostgreSQL-Image mit fest verdrahteten Extension-Versionen), werden auf genau diesen Tag gepinnt. Ebenso Anwendungen, die ihre Daten bei jedem Minor migrieren und ohne Migration nicht starten: **Meilisearch** wird deshalb auf den Minor gepinnt (`v1.51`) und zusätzlich mit `MEILI_UPGRADE_DB=true` betrieben — das Pinnen verhindert Migrationen durch zufällige Image-Updates, das Flag lässt die Migration bei einem bewussten Versionswechsel durchlaufen, statt den Container mit `database version incompatible` beenden zu lassen. Bei aktueller Datenbank ist das Flag wirkungslos.
...
- **Mailversand:** Templates fragen keine SMTP-Zugangsdaten ab, sondern deklarieren eine Delivery-Box (`deliveryBoxes: [{ purpose: main }]`). Die `docker-compose.yml` verdrahtet `mail.agenturserver.de` auf Port `587` mit STARTTLS fest und nutzt `${MW_DELIVERYBOX_MAIN_USERNAME}` / `${MW_DELIVERYBOX_MAIN_PASSWORD}`. Die Absenderadresse bleibt als `SMTP_FROM` ein `userInput` im Step `common`. Ausnahme: `mailpit` betreibt den SMTP-Endpunkt selbst, dessen Zugangsdaten bleiben Eingabefelder. Ausnahme: `nextcloud-euro-office` setzt die Absenderadresse aus `MAIL_FROM_ADDRESS` und `MAIL_DOMAIN` zusammen, weil Nextcloud den Teil vor und hinter dem `@` in getrennten Variablen erwartet (`mail_from_address`, `mail_domain`) und sich ein `SMTP_FROM` in der `docker-compose.yml` nicht aufteilen lässt.
